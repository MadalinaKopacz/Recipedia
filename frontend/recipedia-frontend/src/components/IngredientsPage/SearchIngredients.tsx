import { Button, Grid, ListItem, Stack, TextField, Typography } from "@mui/material";
...
export default function SearchIngredients() {
    ...
    const onAddIng = (searchTerm : string) => {
        axios.get("/api/food-database/v2/parser", {
            params: {
            ingr: searchTerm,
            app_id: ENV.FOOD_APP_ID,
            app_key: ENV.FOOD_APP_KEY,
        ...
        .then((response) => {
            ...
            if(response.data.parsed.length === 0)
                ...
                setUnknownIng(false);
                if(!chosenIng.includes(searchTerm))
                    chosenIng.push(searchTerm);
            }
            setValue("");
            setIngSuggestions([]);
        })
                ...
                <Grid item xs={11}>
                    <Stack 
                        ...
                        {ingSuggestions.map((suggestion) => 
                            <ListItem
                                 onClick={()=> {setValue(suggestion); setIngSuggestions([]);}}
